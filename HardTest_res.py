...
class UrTube:
    ...
    def log_in(self, nickname, password):  # Метод для входа в аккаунт
        hashed_password = int(hashlib.sha256(password.encode()).hexdigest(),
                              16)  # Хэшируем пароль и сравниваем с хэшем из базы
        for user in self.users:  # Проверяем всех пользователей
            if user.nickname == nickname and user.password == hashed_password:  # Если пароли совпадают, авторизуем пользователя
                ...
                return
        print("Неверный логин или пароль")  # Сообщение об ошибке

    def register(self, nickname, password, age):
        if any(user.nickname == nickname for user in
               self.users):  # Проверяем, существует ли пользователь с таким никнеймом
    ...
    def get_videos(self, keyword):  # Метод для получения видео по ключевому слову
        ...
        return [video.title for video in self.videos if
                keyword_lower in video.title.lower()]  # Возвращаем список найденных видео
    ...
    def watch_video(self, title):  # Метод для просмотра видео
        if not self.current_user:  # Если пользователь не авторизован, выходим из функции
            print("Войдите в аккаунт, чтобы смотреть видео")  # Сообщение об ошибке
            return
        ...
        while video.time_now < video.duration:  # Воспроизводим видео до конца
            print(video.time_now + 1)
            time.sleep(0.1)  # Пауза в 0.1 секунду для имитации просмотра
            video.time_now += 1
